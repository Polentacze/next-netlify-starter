...
{/* 🌿🪸 UNIFIED ANCHORED PROP RENDERER: Uses the coral's blueprint to bury the kelp deep into the mud */}
{propsList.kelp.map((k, idx) => {
  // 🧭 Blueprint update: applies the successful coral sinking math (12px) to ALL seabed props!
  const anchorAdjustment = 12;
  ...
  return (
    <img 
      ...
      style={{ 
        position: 'absolute', 
        top: finalTopY, // Locks both props deep below the muddy baseline edge
        left: k.x, 
        ...
        <>
          ...
          <main style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', zIndex: 10 }}>
            ...
            <form className="launch-form" onSubmit={(e) => { e.preventDefault(); setIsPlaying(true); }}>
              <div className="input-wrap"><img src="/input-box.png" alt="Input" style={{ width: '100%' }} /><input type="text" className="field-text" value={username} onChange={(e) => setUsername(e.target.value)} maxLength={25} placeholder="Enter Name..." style={{ color: '#333' }} /></div>
