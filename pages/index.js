...
export default function Home() {
  ...
  // 🩸 STETHACANTHUS ACTIVE ABILITY TRACKERS
  const [isAbilityActive, setIsAbilityActive] = useState(false)
  const holdTimerRef = useRef(null)
  const abilityDurationTimeoutRef = useRef(null)
  ...
  // ⚡ ABILITY BAR STATE TRACKERS (Holds up to 100% full charge)
  ...
  // 💥 MOUSE HOLD: Fills your ability bar when holding the click button
  const handleMouseDown = () => {
    if (isAbilityActive || activeTierIndex !== 1) return

    chargeIntervalRef.current = setInterval(() => {
      setChargeProgress((prev) => {
        if (prev >= 100) return 100 // Caps safely at maximum charge
        return prev + 4 // Fills the bar steadily over 1.5 seconds
      })
    }, 60)
  }

  // 💥 MOUSE LET GO: Fires the skill if the bar reached 100% full
  const handleMouseUp = () => {
    ...
    setChargeProgress((currentCharge) => {
      if (currentCharge >= 100) {
        // SUCCESS: Bar was full! Unleash the 1.5x speed boost for 6 seconds
        setIsAbilityActive(true)
        setTimeout(() => {
          setIsAbilityActive(false)
        }, 6000)
      } else {
        // SHORT CLICK FALLBACK: If released early, check if we can do a normal boost
        if (boostBars > 0 && !isBoosting && !isAbilityActive) {
          setIsBoosting(true)
          setBoostBars(0)
          setTimeout(() => { setIsBoosting(false) }, 320)
        }
      }
      return 0 // Resets the charge bar instantly back to empty
    })
  }
    const detectTextColor = (targetString) => {
    const cleanStr = (targetString || "").toUpperCase()
  ...
  // Mouse Down Event: Checks for Tap Boost vs Long Press Hold Ability
  const handleViewportMouseDown = () => {
    if (activeTierIndex === 1 && !isAbilityActive) {
      holdTimerRef.current = setTimeout(() => {
        setIsAbilityActive(true)
        // Auto-terminate the agility multiplier state after exactly 6 seconds
        abilityDurationTimeoutRef.current = setTimeout(() => {
          setIsAbilityActive(false)
        }, 6000)
      }, 350) // 350ms Hold threshold definition
    }
  }

  // Mouse Up Event: Fires Standard Boost if user performed a regular tap
  const handleViewportMouseUp = () => {
    if (holdTimerRef.current) {
      clearTimeout(holdTimerRef.current)
      holdTimerRef.current = null
    }
    if (isBoosting || isAbilityActive) return
    if (boostBars >= 1) {
      setIsBoosting(true)
      setBoostBars(0)
      setTimeout(() => { setIsBoosting(false) }, 320)
  ...
  useEffect(() => {
    ...
    const tick = setInterval(() => {
      ...
      setPlayerPosition((p) => {
        ...
        // SPEED CONTROLLER: Doubles speed if ability is active, or handles standard speeds
        let spd = dist > 25 ? Math.min(dist * 0.035, 4.8) : 0
        if (isBoosting) spd = 18
        if (isAbilityActive) spd = spd * 1.5 // 1.5x speed modifier active!
    ...
    return () => { 
      ...
      clearInterval(tick)
      if (abilityDurationTimeoutRef.current) clearTimeout(abilityDurationTimeoutRef.current)
    }
  ...
  return (
    <div style={{ textAlign: 'center', padding: '2rem', color: '#FFFFFF', minHeight: '100vh', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', backgroundColor: '#104E8B', position: 'relative', overflowX: 'hidden', userSelect: 'none' }}>
      ...
      {isPlaying ? (
        <div className="arena-viewport" ref={viewRef} onMouseDown={handleMouseDown} onMouseUp={handleMouseUp}>
          ...
          <button className="leave-btn" style={{ right: '20px' }} onClick={() => { setIsPlaying(false); setScore(0); setActiveTierIndex(0); setPendingEvolutionIndex(null); setChargeProgress(0); setIsAbilityActive(false); }}>Leave Map</button>
          ...
          <div className="infinite-ocean-world" style={{ transform: 'translate(' + (400 - playerPosition.x) + 'px, ' + (300 - playerPosition.y) + 'px)' }}>
            ...
            <div style={{ position: 'absolute', top: playerPosition.y, left: playerPosition.x, transform: 'translate(-50%, -50%)', display: 'flex', flexDirection: 'column', alignItems: 'center', width: evoTiers[activeTierIndex].scale + 'px', pointerEvents: 'none', background: 'transparent', backgroundColor: 'transparent' }}>
              ...
              {chargeProgress > 0 && (
                <div style={{ width: '60px', height: '6px', background: 'rgba(0,0,0,0.5)', border: '1px solid white', borderRadius: '3px', marginBottom: '4px', overflow: 'hidden' }}>
                  <div style={{ width: chargeProgress + '%', height: '100%', background: '#ff4d4d' }} />
                </div>
              )}
              {isAbilityActive && activeTierIndex === 1 && (
                <img src="/steth-ability.png" alt="Speed Surge Active" style={{ width: '40px', height: 'auto', marginBottom: '4px', filter: 'drop-shadow(0 0 6px #ff4d4d)' }} onError={(e) => { e.target.src = "/prehistoric-skeleton.png" }} />
              )}
