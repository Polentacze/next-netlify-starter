...
const [knightiaNpcs, setKnightiaNpcs] = useState([
  { id: 1, spawnX: 1400, spawnY: 800, x: 1400, y: 800, angle: 0,   speed: 9, scale: 38, hp: 50, maxHp: 50, lastHit: 0 },
  { id: 2, spawnX: 1450, spawnY: 820, x: 1450, y: 820, angle: 90,  speed: 9, scale: 38, hp: 50, maxHp: 50, lastHit: 0 },
  { id: 3, spawnX: 1380, spawnY: 780, x: 1380, y: 780, angle: 180, speed: 9, scale: 38, hp: 50, maxHp: 50, lastHit: 0 },
  { id: 4, spawnX: 1420, spawnY: 840, x: 1420, y: 840, angle: 270, speed: 9, scale: 38, hp: 50, maxHp: 50, lastHit: 0 },
...
setPropsList({
  kelp: [
    ...
    { x: 1050, y: 1739, h: 85, type: 'white-brain-coral' }, // ADDED HERE INSIDE ARRAY
...
if (isBoosting) {
  if (activeTierIndex === 1) {
    ...
    spd = isAbilityActive ? 23 : 18
        ...
        cy = Math.max(50, Math.min(1715, p.y + dy))
        return { x: cx, y: cy }
      })
...
<div key={i} className="chat-msg-row" style={{ color: (m.colorCode === '#FFFFFF' || !m.colorCode) ? '#FFFFFF' : m.colorCode }}>
    <strong style={{ 
    ...
    {m.text}
              ...
              <form onSubmit={handleSendChat}>
                <input type="text" className="chat-input-bar-inner" placeholder="Press Enter to type in chat..." value={chatInput} onChange={(e) => setChatInput(e.target.value)} maxLength={45} />
              </form>
            </div>
          ) : (
  ...
  return (
    ...
    />
            ...
            <div style={{ position: 'absolute', top: playerPosition.y, left: playerPosition.x, transform: 'translate(-50%, -50%)', display: 'flex', flexDirection: 'column', alignItems: 'center', width: evoTiers[activeTierIndex]?.scale + 'px', pointerEvents: 'none', background: 'transparent', backgroundColor: 'transparent' }}>
              <span style={{ background: 'transparent', padding: '2px 8px', borderRadius: '4px', fontSize: '0.75rem', fontWeight: 'bold', fontFamily: 'sans-serif', marginBottom: '8px', border: 'none' + (detectTextColor(username) !== '#FFFFFF' ? detectTextColor(username) : '#00FF1A'), color: detectTextColor(username), whiteSpace: 'nowrap' }}>{username || "Guest"}</span>
...
<img
  ...
  onError={(e) => { e.target.src = "/prehistoric-skeleton.png?v=8" }}
  alt="critter"
...
>
  ...
  <button type="submit" className="play-btn" style={{ margin: 0, width: '160px' }}>
