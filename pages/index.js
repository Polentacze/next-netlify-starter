...
export default function Home() {
  ...
  const handleViewportClick = () => {
    ...
    const pellets = []
    for (let c = 0; c < 8; c++) {
      const centerX = Math.floor(Math.random() * 2600) + 200, centerY = Math.floor(Math.random() * 1400) + 200
      for (let i = 0; i < 6; i++) pellets.push({ id: "s_" + c + "_" + i, x: centerX + (Math.random() * 120 - 60), y: centerY + (Math.random() * 120 - 60), isEaten: false, value: 100, src: "/food.png" })
    }
    for (let c = 0; c < 4; c++) {
      const centerX = Math.floor(Math.random() * 2600) + 200, centerY = Math.floor(Math.random() * 1400) + 200
      for (let i = 0; i < 4; i++) pellets.push({ id: "p_" + c + "_" + i, x: centerX + (Math.random() * 120 - 60), y: centerY + (Math.random() * 120 - 60), isEaten: false, value: 120, src: "/ocean-food.png" })
    }
  ...
  useEffect(() => {
    ...
    const gameLoop = setInterval(() => {
      let currentX = playerPosition.x, currentY = playerPosition.y
      setPlayerPosition((p) => {
        const angleRad = Math.atan2(mousePos.current.y, mousePos.current.x), distance = Math.sqrt(mousePos.current.x ** 2 + mousePos.current.y ** 2)
        let speedMultiplier = distance > 25 ? Math.min(distance * 0.05, 8) : 0
        if (isBoosting) speedMultiplier = 28 
        const dx = Math.cos(angleRad) * speedMultiplier, dy = Math.sin(angleRad) * speedMultiplier
        if (speedMultiplier > 0) { setPlayerRotation(angleRad * (180 / Math.PI) + 90) }
        currentX = Math.max(50, Math.min(2950, p.x + dx)); currentY = Math.max(50, Math.min(1725, p.y + dy))
        return { x: currentX, y: currentY }
      })
      setFoodPellets((prevPellets) =>
        prevPellets.map((pellet) => {
          if (pellet.isEaten) return pellet
          if (Math.sqrt((currentX - pellet.x) ** 2 + (currentY - pellet.y) ** 2) < 30) {
            setScore((s) => s + pellet.value)
            setFoodEatenCount((prev) => { const next = prev + 1; if (next >= 5) { setBoostBars((b) => Math.min(3, b + 1)); return 0 }; return next })
            setTimeout(() => {
              setFoodPellets((currentPellets) => currentPellets.map((p) => {
    ...
    return () => { window.removeEventListener('mousemove', handleMouseMove); clearInterval(gameLoop) }
  }, [isPlaying, playerPosition, isBoosting])
      return (
      ...
      {isPlaying ? (
        <div className="arena-viewport" ref={viewRef} onMouseDown={handleViewportClick}>
          ...
          <div className="infinite-ocean-world" style={{ transform: 'translate(' + (400 - playerPosition.x) + 'px, ' + (300 - playerPosition.y) + 'px)' }}>
            ...
            {/* FIXED HITBOX CONTAINER LOWER CLOSING STRIP */}
